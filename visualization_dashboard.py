...
mobile_nodes = list(set(e['node'] for e in events if e['type'] in ['INITIAL', 'MOVE']))
...
def update(frame):
    ...
    if event['type'] == 'INITIAL':
        ...
        if status == 'ATTACKER':
            ...
            status_text.set_text(f"ALERT: Suspicious movement detected for {node.replace('_', ' ').title()}!")
        else:
